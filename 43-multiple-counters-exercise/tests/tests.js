...
describe("2. timer should continuously count up", () => {
	...
	test("should display time elapsed", async () => {
		...
		const firstTimer = document.querySelector(".timers .timer_0");

		const secondsDisplayed = firstTimer.querySelector("h3").innerText
...
describe("3. reset timer", () => {
	...
	test("should reset timer on click", async () => {
		...
		expect(firstTimerH3.innerText).toEqual("2");

		fireEvent.click(firstTimerReset);
		await new Promise((resolve) => setTimeout(resolve, 100));
		expect(firstTimerH3.innerText).toEqual("0");
	});
});
...
describe("4. stop timer", () => {
	...
	test("should stop timer on click", async () => {
		...
		expect(firstTimerH3.innerText).toEqual("1");
...
describe("5. remove timer", () => {
	...
	test("should remove timer on click", () => {
		...
		expect(Boolean(document.querySelector(".timers .timer_0"))).toBe(true);

		fireEvent.click(firstTimerCross);
		//the timers had to be renamed because they were getting created with wrong indexes
		expect(Boolean(document.querySelector(".timers .timer_0"))).toBe(true);
